...
#include "Dungeon/TRRoomCustomData.h"
...
protected:
	...
	// 새 공용 키 - 문 페어 인스턴스를 생성한다
	// 성공 여부를 반환한다
	bool SpawnDoorKeyMultiInstancePair(URoom* DoorRoom, URoom* KeyRoom, ADoor* LockDoor);
...
protected:
	...
	// NOTE: 공용 키의 경우 플레이어 이해를 위해 ID와 무관하게 외형을 통일하는 것(=어레이에 값 하나만 넣는 것)이 권장된다
	UPROPERTY(EditDefaultsOnly)
	TArray<FLockedDoorKeyPairInfo> DoorKeyMultiGenInfos;

	// TODO
	UPROPERTY(EditDefaultsOnly)
	TArray<FLockedDoorKeyPairInfo> DoorKeySingleGenInfos;
...
private:
	...
	// 공용 키 - 문 페어를 생성할 때 이 값을 토대로 아이디를 issue한다
	// 공용키는 아이디와 관계없이 공용키 하나를 소비해 공용키로 잠긴 문 하나를 아무거나 열 수 있다
	// 공용 키이더라도 아이디는 각자 모두 달라야 하며, TR_MAX_MULTIKEY_LOCKED_DOORS_PER_LEVEL를 넘어서는 안된다
	int32 NewDoorKeyMultiId = 0;

	// TODO: 기능 추가
	// 개인 키 - 문 페어를 생성할 때 이 값을 토대로 아이디를 issue한다
	// 개인 키로 잠긴 문은 같은 아이디의 개인 키로만 열 수 있다
	// 공용키와 개인키는 같은 어레이에 저장되기 때문에,
	// 이 값이 서로 중첩되지 않게 하기 위해 아이디 값이 반드시 공용키 ID의 최댓값을 넘어야 한다
	int32 NewDoorKeySingleId = TR_MAX_MULTIKEY_LOCKED_DOORS_PER_LEVEL + 1;
...
public:
	...
	TSet<URoom*> Server_GetRoomsOfOcculusionDepth(uint32 Depth);
	...
	TPair<TSet<URoom*>, TSet<URoom*>> Server_GetRoomsOfOcculusionDepthInBetween(uint32 MinDepth, uint32 MaxDepth);

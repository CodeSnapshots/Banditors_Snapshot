...
public:
	...
	const bool ShouldDisplayUI() const { return bDisplayUI; }
...
public:
	...
	// 주어진 델타를 레퍼런스 카운트에 더한다 (혹은 음수를 사용해 카운트를 줄인다)
	// bUpdateValidity가 true인 경우, 레퍼런스 카운트가 0이 되면 즉각 Invalidate한다
	void Server_AddDeltaRefCount(int16 DeltaOwningItemRefCount, int16 DeltaWieldingItemRefCount, bool bUpdateValidity);
...
protected:
	// 이 상태이상을 부여중인 소지(인벤토리) 아이템 수
	// 동일한 상태이상이 서로 다른 아이템들에 의해 부여될 수 있음
	int16 OwningItemRefCount = 0;

	// 이 상태이상을 부여중인 장착 아이템 수
	int16 WieldingItemRefCount = 0;

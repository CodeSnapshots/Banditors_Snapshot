...
public:	
	...
	// 캐싱에 사용할 인벤토리 정보를 외부로 export한다
	// 내부 값들(Outer등)이 수정되기 때문에, 더이상 해당 컴포넌트 인스턴스가 불필요한 경우에만 사용해야 한다
	// 인벤토리의 복구에 필요한 필수 데이터들에 한해서만 정보를 추출한다
	// Keys의 경우 인벤토리를 복구하는 과정에서 새로 할당되기 때문에 전달이 불필요하다
	void Server_ExportInventoryInfo(TArray<class UInvObject*>* OutValues, TArray<FInvObjData>* OutDatas, UObject* PersistentOuter);

...
public:
	...
	int32 GetStat_MaxHealth() const;
	UFUNCTION(BlueprintPure, Category = "Health")
	int32 GetStat_CurrentHealth() const { return HealthPoint; }
	UFUNCTION(BlueprintPure, Category = "Health")
	float GetStat_CurrHealthPercentage() const { return (float)GetStat_CurrentHealth() / (float)GetStat_MaxHealth(); }

	// Setters
	UFUNCTION(BlueprintCallable, Category = "Health")
	void Server_SetPureMaxHealthTo(const int32 Value);
	UFUNCTION(BlueprintCallable, Category = "Health")
	void Server_SetHealthPointTo(const int32 Value);
...
protected:
	...
	UPROPERTY(ReplicatedUsing = OnRep_AppliedStatEffects)
...
protected:
	...
	// 상태이상 변경 시 모든 호스트들에서 공통으로 처리해야 하는 로직을 처리한다
	void Host_OnStatEffectChanged();

	// 값 갱신 시 로직 처리
	UFUNCTION()
	void OnRep_AppliedStatEffects();
...
public:
	...
	void Local_SwitchAnimClassLayer(EAnimClassType ClassType);
...
public:
	...
	// 슬라이딩 FX를 재생한다
	// NOTE: 현재 슬라이딩 FX는 오직 FPSCharacter에게만 유효하게 동작한다
	virtual void Local_SetSlideFx(bool bValue) { return; }
...
private:
	...
	// 현재 이 캐릭터에 바인딩된 Boss state UI
	// 보스 몬스터들에 한해 바인딩된다
	TWeakObjectPtr<class UBossStateWidget> Local_BossStateWidget = nullptr;
#pragma endregion

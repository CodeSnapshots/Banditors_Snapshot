...
protected:
	...
	// Hard, Seamless 트랜지션 후 공통 로직
	void PostAnyTravel();

	// 모든 호스트의 Seamless 트랜지션 완료 후 월드 로드 완료 시 로직
	// 1회만 호출된다
	void PostAllHostWorldLoaded();
...
protected:
	...
	// 현재 레벨에서 PostAllHostWorldLoaded가 호출되었는지 여부
	bool bPostAllHostWorldLoadedCalled = false;
...
public:
	// 레벨 변경 요청을 비동기적으로 처리한다
	// 처리 여부를 반환
	bool AsyncChangeGameLevel(FString LevelURL, int32 NewDepth = -1);
...
public:
	// 트랜지션 스테이트
	// 트랜지션이 완료될 때마다 초기화되어야 한다
	bool bReadyToTravel = false;
	int32 PreparedHostCount = 0;
	int32 TotalHostCountCached = 1;

protected:
	FTimerHandle AsyncLevelChangeTimer;

	// 현재 비동기적으로 레벨 전환 처리중인지 여부
	// 처리중이라면 1 이상의 값을 가지며, 정확히는 현재 처리를 위해 재귀한 횟수를 구함
	int32 AsyncLevelChangeTrialCount = 0;

	// 클라이언트가 도중에 튕기거나 하는 상황이 발생해 뭔가 숫자가 안맞는 경우 등
	// 일정 시도 이후부터는 이러한 오류로 간주해 더이상의 시도를 중단함
	const int32 MaxAsyncLevelChangeTrialCount = 8;

	// 재시도 주기 (sec)
	const float AsyncLevelChangeTrialRate = 0.5f;
...
public:
	...
	// 주어진 위치에서 가장 가까운, 누군가 컨트롤중인 플레이어 캐릭터를 찾아 반환한다
	AFPSCharacter* FindClosestActivePlayerChar(FVector BaseLocation);
#pragma endregion
...
public:
	...
	// 현재 Occulusion 시야 범위 + 해당 값의 범위가 유효 스폰 구역이다
	UPROPERTY(EditDefaultsOnly)
	uint32 SpawnableDepthOffset = 1;

	// 유효 깊이 오프셋
	UPROPERTY(EditDefaultsOnly)
	uint32 ValidDepthOffset = 1;
	...
	// 스폰 웨이브는 동적으로 결정됨
	UPROPERTY(EditDefaultsOnly)
	float DespawnWaveCycle = 1.0f;

	UPROPERTY(EditDefaultsOnly)
	float DeactivateWaveCycle = 0.5f;

	// 단일 웨이브에 디스폰 가능한 최대 봇 개수
	UPROPERTY(EditDefaultsOnly)
	int32 MaxDespawnedBotPerCycle = 3;
...
protected:
	...
	float TimeSinceLastDeactivateWave = 0.0f;
...
private:
	// 계층 별 기준치
	// 초기화는 StartPlay에서 처리한다
	uint32 InSightDepth = 0;
	uint32 SpawnableDepth = 0;
	uint32 ValidDepth = 0;
...
protected:
	...
	void SpawnWave(const TSet<class URoom*>& SpawnRooms);
	void DespawnWave(const TSet<class URoom*>& ValidRooms);
	void DeactivateWave(const TSet<class URoom*>& ActiveRooms);

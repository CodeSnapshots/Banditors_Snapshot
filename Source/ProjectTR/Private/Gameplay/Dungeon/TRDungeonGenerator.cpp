...
#include "DungeonActors/DoorBlocker.h"
...
void ATRDungeonGenerator::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
...
void ATRDungeonGenerator::Server_GenerateLockedDoors()
{
	...
	int32 MaxLockedPairCount = FMath::Clamp(GetNbRoom() / 2, TR_MIN_MULTIKEY_LOCKED_DOORS_PER_LEVEL, TR_MAX_MULTIKEY_LOCKED_DOORS_PER_LEVEL);
	...
	for (int32 TrialNum = 0; TrialNum < GenTrialCount; ++TrialNum)
	{
		...
		if (!SpawnDoorKeyMultiInstancePair(LockedRoom, KeyRoom, LockDoor))
		{
			continue;
		}
...
bool ATRDungeonGenerator::SpawnDoorKeyMultiInstancePair(URoom* DoorRoom, URoom* KeyRoom, ADoor* LockDoor)
{
	...
	if (!World)
	{
		UE_LOG(LogTemp, Error, TEXT("SpawnDoorKeyMultiInstancePair - Invalid world!"));
		return false;
	}
	...
	if (PairId < 0 || DoorKeyMultiGenInfos.IsEmpty())
	{
		UE_LOG(LogTemp, Error, TEXT("SpawnDoorKeyMultiInstancePair - Something went wrong!"));
		return false;
	}

	const FLockedDoorKeyPairInfo& PairGenInfo = DoorKeyMultiGenInfos[FMath::Min(DoorKeyMultiGenInfos.Num() - 1, PairId)];
	...
	if (!bSpawnLocFound)
	{
		UE_LOG(LogTemp, Error, TEXT("SpawnDoorKeyMultiInstancePair - Unable to find valid key spawn point."));
		return false;
	}
	...
	if (!KeySpawned)
	{
		UE_LOG(LogTemp, Error, TEXT("SpawnDoorKeyMultiInstancePair - Key spawn failed!"));
		return false;
	}
	...
	if (!DoorSpawned)
	{
		UE_LOG(LogTemp, Error, TEXT("SpawnDoorKeyMultiInstancePair - Door spawn failed!"));
	...
	NewDoorKeyMultiId++;
	check(NewDoorKeyMultiId <= TR_MAX_MULTIKEY_LOCKED_DOORS_PER_LEVEL);
	return true;
}
...
TSet<URoom*> ATRDungeonGenerator::Server_GetRoomsOfOcculusionDepth(uint32 Depth)
{
	TSet<URoom*> Results;
	Results.Empty();
	if (!HasAuthority())
...
TPair<TSet<URoom*>, TSet<URoom*>> ATRDungeonGenerator::Server_GetRoomsOfOcculusionDepthInBetween(uint32 MinDepth, uint32 MaxDepth)
{
	TSet<URoom*> Results;
	Results.Empty();
	if (!HasAuthority())
...
void ATRDungeonGenerator::Local_ToggleLoadingScreenWidget(bool bVisibility)
{
	if (bVisibility && LoadingScreenWidgetClass)
	{
		...
		if (LoadingScreenWidget)
		{
			LoadingScreenWidget->AddToViewport(WZO_LOADING);
		}
		return;
	}
	...
	if (!bVisibility)
	{
		// NOTE: RemoveFromParent는 유효하지 않을 수 있다 (GC에 의해 제거된 경우)

...
void UInventoryComponent::Server_ExportInventoryInfo(TArray<class UInvObject*>* OutValues, TArray<FInvObjData>* OutDatas, UObject* PersistentOuter)
{
	...
	for (int32 Index = 0; Index < InvObjectKeys.Num(); ++Index)
	{
		...
		if (InvObjectValues[Index])
		{
			...
			// 트랜지션 과정의 GC를 피하기 위해 아우터를 트랜지션 과정에서 살아남는 오브젝트로 변경 + RootSet에 추가
			CValue->ChangeOuterRecursive(PersistentOuter, true);
		}
...
void UInventoryComponent::Server_ImportInvComp(TArray<class UInvObject*> NewValues, TArray<FInvObjData> NewDatas)
{
	...
	// 아우터를 다시 원래 상태로 복구한다 (캐릭터)
	// 또한 오브젝트들을 루트셋으로부터 제거한다
	// 이는 메모리 리킹을 방지하기 위함으로, 이 과정은 임포트의 최상단부에서 진행하는 게 권장된다
	// 루트셋 리킹 발생으로 크래시가 날 수 있기 때문
	for (UInvObject* DeletePendingObj : NewValues)
	{
		DeletePendingObj->ChangeOuterRecursive(GetOwner(), false);
	}
	...
	if (CharOwner && CharOwner->HasAuthority())
	{
		...
		if (AddedObj->bApplyStatusEffectToOwner)
		{
			...
			if (OwnerStatusEffect)
			{
				...
				OwnerStatusEffect->Server_AddDeltaRefCount(1, 0, true); // Weakptr 생성 이후 처리
			}
			else
			{
...
void UInventoryComponent::Server_OnInvObjectRemoved(UInvObject* RemovedObj)
{
	...
	if (CharOwner && CharOwner->HasAuthority())
	{
		if (RemovedObj->CachedOwnerStatEffect.IsValid())
		{
			RemovedObj->CachedOwnerStatEffect.Get()->Server_AddDeltaRefCount(-1, 0, true);
		}
	}
}

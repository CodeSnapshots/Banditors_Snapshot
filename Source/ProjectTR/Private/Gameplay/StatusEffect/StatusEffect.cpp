...
void UStatusEffect::ChangeOuterRecursive(UObject* NewOuter)
{
...
void UStatusEffect::Server_AddDeltaRefCount(int16 DeltaOwningItemRefCount, int16 DeltaWieldingItemRefCount, bool bUpdateValidity)
{
	OwningItemRefCount += DeltaOwningItemRefCount;
	WieldingItemRefCount += DeltaWieldingItemRefCount;
	
	if (bUpdateValidity && OwningItemRefCount <= 0 && WieldingItemRefCount <= 0)
	{
		// 타이머가 있을 경우 Invalidation은 타이머에 맞김
		if (GetWorld() && GetWorld()->GetTimerManager().IsTimerActive(Server_DurationTimer))
		{
			return;
		}
		// 아닐 경우 즉각 Invalidate

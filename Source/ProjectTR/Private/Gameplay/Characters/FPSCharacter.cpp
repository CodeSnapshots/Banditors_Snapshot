...
////////////TESTING
#include "DungeonActors/TRChest.h"
...
#pragma region /** Component Initialization */
	...
	// 나이아가라
	SlideVFXComp = CreateDefaultSubobject<UNiagaraComponent>(TEXT("NiagaraSlide"));
	check(SlideVFXComp != nullptr);
	SlideVFXComp->SetupAttachment(GetCapsuleComponent());
	SlideVFXComp->SetAutoActivate(false);
	SlideVFXComp->SetRelativeLocation(FVector::DownVector * GetCapsuleComponent()->GetScaledCapsuleHalfHeight()); // 세부 위치 조정 필요
	SlideVFXComp->bUseAttachParentBound = true;
#pragma endregion
...
void AFPSCharacter::BeginPlay()
{
	...
	if (SlideVFX)
	{
		SlideVFXComp->SetAsset(SlideVFX);
	}
	...
	// 핑
	AimedTargetPingCollisionParams.AddIgnoredActor(GetOwner());
...
void AFPSCharacter::CheckGameInfo(const FInputActionValue& Value)
{
	...
	if (GetWorld())
	{
		...
		{
			// 로컬, 사망한 캐릭터 제외
			if (!(*It) || (*It)->IsLocallyViewed() || (*It)->GetHasDied()) continue;
			Local_HighlightTarget(*It, true, false);
		}
	}

	// 위젯 처리
	if (Local_GameInfoWidget.IsValid())
	{
		UE_LOG(LogTemp, Error, TEXT("CheckGameInfo - GameInfoWidget already exists! This could be an input buffer issue or memory leak."));
		return;
	}
	ATRPlayerController* PC = Cast<ATRPlayerController>(GetController());
	if (PC && PC->IsLocalController())
	{
		UUserWidget* NewWidget = PC->Local_CreateWidget(PC->GameInfoWidgetClass, this);
		UTeamStateWidget* GameInfoWidget = Cast<UTeamStateWidget>(NewWidget); // NOTE: 추후 TeamState 외의 정보가 필요한 경우 교체
		if (GameInfoWidget)
		{
			// 초기화
			Local_GameInfoWidget = GameInfoWidget;
			GameInfoWidget->Local_Init();

			PC->Local_DisplayWidget(GameInfoWidget, WZO_GAMEINFO);

			// NOTE: 추후 게임 정보 위젯이 인풋을 사용해야 할 경우 포커스를 줄 수 있음
			// 단, 이 경우 토글해제 인풋에 대한 처리를 캐릭터가 아닌 위젯에서 처리해야 함
			// PC->Local_FocusWidget(GameInfoWidget, true, true);
		}
		else if (NewWidget)
		{
			UE_LOG(LogTemp, Error, TEXT("CheckGameInfo - GameInfoWidget has wrong widget type!"));
			PC->Local_DerefWidget(NewWidget);
		}
	}
}
...
void AFPSCharacter::StopCheckGameInfo(const FInputActionValue& Value)
{
	...
	if (GetWorld())
	{
		for (TActorIterator<AFPSCharacter> It(GetWorld()); It; ++It)
		{
			// 해제하기 전까지 상태가 변화했을 수 있기 때문에, 모든 캐릭터에 대해 해제를 처리해준다
			Local_HighlightTarget(*It, false, false);
		}
	}
	...
	if (!Local_GameInfoWidget.IsValid())
	{
		UE_LOG(LogTemp, Error, TEXT("StopCheckGameInfo - GameInfoWidget is already invalid!"));
		return;
	}
	ATRPlayerController* PC = Cast<ATRPlayerController>(GetController());
	if (PC && PC->IsLocalController())
	{
		PC->Local_DerefWidget(Local_GameInfoWidget.Get());
		Local_GameInfoWidget = nullptr;
	}
}
...
FString AFPSCharacter::Host_GetName() const
{
	if (NameWidgetComponent)
	{
		UPlayerNameTagWidget* TagWidget = Cast<UPlayerNameTagWidget>(NameWidgetComponent->GetWidget());
...
void AFPSCharacter::Local_UpdateAimedTargetUI()
{
	...
	// 캐릭터는 에임 아웃라인 대상에서 제외
	Local_HighlightTarget(PrevActor, false, true);
	Local_HighlightTarget(CurrActor, true, true);
	PrevAimedActor = CurrActor;
}

void AFPSCharacter::Local_HighlightTarget(AActor* Target, bool bVisibility, bool bIgnoreCharacters)
{
	if (!Target) return;
	...
	// 아웃라인
	ATRPlayerController* TRPC = GetController<ATRPlayerController>();
	if (TRPC)
	{
		...
		for (UPrimitiveComponent* TargetComp : TargetComps)
		{
			...
			{
				TRPC->Local_EraseOutline(TargetComp, false);
			}
		}
	}
...
void AFPSCharacter::Server_PingAtAim_Implementation()
{
	...
	// 라인 트레이싱
	FHitResult AimedPingHit;
	FVector StartLocation;
	FRotator Rotation;
	GetActorEyesViewPoint(StartLocation, Rotation);
	FVector EndLocation = StartLocation + (Rotation.Vector() * PingDistance);

	bool bHit = World->LineTraceSingleByObjectType(AimedPingHit, StartLocation, EndLocation, AimedTargetPingObjQueryParams, AimedTargetPingCollisionParams);
	if (bHit)
	{
...
void AFPSCharacter::BindAllyState(class UAllyStateWidget* AllyState)
{
	if (!AllyState) return;
	this->Local_AllyStateWidget = AllyState;
}

void AFPSCharacter::UnbindAllyState()
{
	this->Local_AllyStateWidget = nullptr;
}

void AFPSCharacter::Local_SetSlideFx(bool bValue)
{
	if (IsValid(SlideVFXComp))
	{
		if (bValue)
		{
			SlideVFXComp->Activate();
		}
		else
		{
			SlideVFXComp->Deactivate();
		}
	}
}

...
#include "Inventory/SlotComponent.h"
...
#include "Dungeon/TRRoomObserverComponent.h"
#include "Damage/TRDamageType.h"
#include "Damage/DamageTypeNeutral.h"
#include "Damage/DamageTypePhysical.h"
#include "Damage/DamageTypeElemental.h"
#include "Damage/DamageTypeMagical.h"
...
void AGameCharacter::BeginPlay()
{
	...
	if (!HasAuthority())
	{
		...
		// 클라의 경우 캐릭터 BeginPlay에서 현재 장착품에 대해 상태 갱신
		// 아직 액터 자체나 혹은 세부 액터 정보(e.g. 건파츠)가 동기화되지 않았을 수 있고,
		// 이 경우들에도 모두 처리가 가능하도록 Local_OnCurrWeaponActorUpdated를 여러 곳에서 호출중임
		if (EquipSystem)
		{
			EquipSystem->Local_OnCurrWeaponActorUpdated();
		}
	}
...
FGameCharacterInstanceData AGameCharacter::Server_GetInstanceData()
{
	...
	if (InvComponent)
	{
		InvComponent->Server_ExportInventoryInfo(&NewData.InvObjectValues, &NewData.InvObjectDatas, GetGameInstance());
...
bool AGameCharacter::Server_RestoreFromInstanceData(const FGameCharacterInstanceData& InstData)
{
	...
	{
		this->Server_SetPureMaxHealthTo(InstData.PureMaxHealth);
	}

	if (InstData.bHealthPointCached)
	{
		this->Server_SetHealthPointTo(InstData.HealthPoint);
	}
	...
	if (EquipSystem && InstData.bEquipSysCached)
	{
		...
		Server_RegisterSwitchToRPC(InstData.CurrentSlot);
	}
	return true;
}
...
void AGameCharacter::Server_OnStatEffectChanged()
{
	...
	// 서버의 경우 수동으로 호출한다
	Host_OnStatEffectChanged();
}

void AGameCharacter::Host_OnStatEffectChanged()
{
	if (IsLocallyControlled() && Local_GetBoundHUDWidget().IsValid())
	{
		Local_GetBoundHUDWidget()->UpdateStatusEffects();
	}
}

void AGameCharacter::OnRep_AppliedStatEffects()
{
	Host_OnStatEffectChanged();
}
...
{
	Local_OnHealthUpdate();
}

void AGameCharacter::Local_OnHealthUpdate()
{
	// UI 업데이트 (서버,클라)
	...
	if (Local_BossStateWidget.IsValid())
	{
		// 보스몹일 경우 추가 UI 업데이트
		Local_BossStateWidget->Update();
	}
}
...
float AGameCharacter::TakeDamage(float DamageTaken, FDamageEvent const& DamageEvent, AController* EventInstigator, AActor* DamageCauser)
{
	...
	Server_SetHealthPointTo(HealthPoint - ActualDamage);
...
void AGameCharacter::Server_SetPureMaxHealthTo(const int32 Value)
{
	if (HasAuthority())
	{
		PureMaxHealth = Value;
		RealMaxHealthCached = GetStat_MaxHealth(); // 클라에게 새 변경값을 전달하기 위해 캐싱해야 한다
		Server_SetHealthPointTo(HealthPoint); // 변경된 범위에 맞게 재업데이트
	}
}

void AGameCharacter::Server_SetHealthPointTo(const int32 Value)
{
	if (HasAuthority())
	{
...
void AGameCharacter::Local_SwitchAnimClassLayer(EAnimClassType ClassType)
{
	...
	UE_LOG(LogTemp, Error, TEXT("Local_SwitchAnimClassLayer - Character %s failed to switch animclass!"), *(GetName()));
}
...
void AGameCharacter::BindBossState(UBossStateWidget* BossState)
{
	if (!BossState) return;
	this->Local_BossStateWidget = BossState;
}
